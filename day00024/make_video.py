# -*- coding: utf-8 -*-
"""
NEON SWARM Day24 - Short Promo Video
...
VIDEOS = [
    BASE / "2026-04-11 11-04-41.mp4",
    BASE / "2026-04-11 11-05-15.mp4",
    BASE / "2026-04-11 11-05-53.mp4",
]

# (ナレーション, 使用動画インデックス, 開始秒, 終了秒)
SCENES = [
    (
        "今日はNEON SWARM！レーザーが自動で回転して敵を倒してコインを稼ぐゲームだよ！スキルをどんどん強化しよう！",
        0, 0, 12,
    ),
    (
        "ハンターっていうじゃまキャラが出てきて、自分のレーザーでポイントを横取りしてくる！でも体当たりすると毎秒200コイン奪えるから追いかけた方が強いよ！",
        1, 0, 14,
    ),
    (
        "後半は画面外から支援ビームが降り注いだり、敵のコインが10倍になったり、スキルツリーがバカみたいに深くなるよ！概要欄から遊べるよ！",
        2, 0, 13,
    ),
]
...
def main():
    print("=== NEON SWARM Promo Video ===")
    self_check()

    print("\n[1/2] TTS + building clips...")
    clips = []
    for i, (voice, vi, ts, te) in enumerate(SCENES):
        print(f"  Scene {i+1}/{len(SCENES)}: {voice[:25]}...")
