...
public static class BuildMura {
    ...
    public static void Build() {
        ...
        fix.spots = new[] {
            ...
            // バス停（被写体＝バス停の 標識）
            // ★カメラは 自分の 領域の **そと**に 置く（中に 置くと すぐ 横に 来た
            //   主人公が 画角に 入らず、選考落ちして 遠景に さらわれる＝camlogで 実測）
            S("ばすてい",   14f, -52f, 30f, 16f,   22f, 4.5f, -36f,   10f, 1.0f, -55f, 42f),
            // 庭から 下（西）へ 出た ところの 道（被写体＝幹線の 道すじ）。
            // ★家の すぐ そとで めちゃ引きに なる、の 直し：あいだに 寄りの 台を 1つ かます
            // ★南（家の 対角）から 見ると 視線が 家の 角を かすめて 遮蔽あつかいに なる
            //   （camlogの「遮」で 実測）。北がわから 南を 見る 置き方に 変えた
            S("みちみなみ",  27f, -27f, 16f, 26f,   24f, 6.0f, -10f,   26f, 0.5f, -32f, 42f),
            // 田んぼ（被写体＝田と 案山子）：南から 田んぼだけ（寄り）
            // ★引きすぎ（本人）→ 10m まで 寄せた
            S("たんぼ",     10f, -18f, 26f, 20f,   10f, 5.5f, -33f,   10f, 0.5f, -15f, 42f),
            // 飛び石（被写体＝飛び石の 列）
            S("とびいし",   20f, 1f, 24f, 12f,     20f, 4.5f, -10f,   20f, -0.3f, 8f, 42f),
            // 橋（被写体＝橋。すこし 俯瞰）
